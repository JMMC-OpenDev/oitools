...
import fr.nom.tam.util.TableException;
 ...
 */
public final class BinaryTable extends Data implements TableData {
     ...
     * @return byte size corresponding to a single value
     * @throws FitsException if the column keywords are invalid
     */
    private int processCol(Header header, int col) throws FitsException {
        ...
        String tform = header.getTrimmedStringValue("TFORM" + (col + 1));
        if (tform == null) {
            throw new FitsException("Attempt to process column " + (col + 1) + " but no TFORMn found.");
        }

        // LAURENT : use getTrimmedStringValue instead of getStringValue :
        String tdims = header.getTrimmedStringValue("TDIM" + (col + 1));
     ...
     *
     * @param tform trimmed value of the TFORM field
     * @return char representing the FITS type
     */
    char getTFORMType(String tform) {

        for (int i = 0, len = tform.length(); i < len; i += 1) {
    ...
    /**
     * Get an unsigned number at the beginning of the TFORM field
     * @param tform trimmed value of the TFORM field
     * @return unsigned number at the beginning of the TFORM field
     */
    private int initialNumber(String tform) {

        int i, len = tform.length();
        for (i = 0; i < len; i += 1) {
            if (!Character.isDigit(tform.charAt(i))) {
                break;
            }
    ...
    public static int[] getTDims(String tdims) {
        ...
        if (first >= 0 && last > first) {
            ...
            StringTokenizer st = new StringTokenizer(tdims, ",");
...
//        if (dimens[col].length > 0 && !isVarCol(col)) {
        if (dimens[col].length > 1 && !isVarCol(col)) {
            ...
            tdim.append(')');
            key = "TDIM" + (col + 1);
            iter.add(key, new HeaderCard(key, tdim.toString(), "ntf::binarytable:tdimN:1"));
        }
    }
