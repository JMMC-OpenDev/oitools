 ...
 * for the user.
 */
public final class Header implements FitsElement {
    ...
    /** Specify where long strings are enabled. */
     ...
     * 		blanks stripped.
     */
    public final String getTrimmedStringValue(final String keyword) {
        final String s = getStringValue(keyword);
        if (s != null) {
            return s.trim();
        }
        return null;
    ...
    public void read(ArrayDataInput dis)
        ...
        boolean isRandom = (dis instanceof RandomAccess);
        if (isRandom) {
        ...
        try {
            while (true) {
                ...
                try {
                    ...
                    //
                    if (firstCard && ((need == 80) || FitsFactory.getAllowTerminalJunk() && (!isRandom || (fileOffset > 0)))) {
                        throw e;
                    }
                ...
                if (firstCard) {
                    ...
                    if (key == null || (!key.equals("SIMPLE") && !key.equals("XTENSION"))) {
                        if (FitsFactory.getAllowTerminalJunk() && (!isRandom || (fileOffset > 0))) {
    ...
    public void setXtension(String val) {
        ...
        try {
            iter.add("XTENSION",
                    new HeaderCard("XTENSION", val, "ntf::header:xtension:" + val));
        } catch (HeaderCardException e) {
    ...
    public void setNaxis(int axis, int dim) {
        ...
        try {
            iter.add("NAXIS" + axis,
                    new HeaderCard("NAXIS" + axis, dim, "ntf::header:naxisN:" + axis));
...
//            if (!card.isKeyValuePair() && card.getKey().equals("END")) {
    ...
    Cursor positionAfterIndex(String prefix, int col) {
        ...
        if (iter.hasNext()) {
            ...
            // Bug fix (references to forward) here by Laurent Bourges
