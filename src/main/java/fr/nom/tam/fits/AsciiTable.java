...
import fr.nom.tam.util.FormatException;
...
public final class AsciiTable extends Data implements TableData {
    ...
    public AsciiTable(Header hdr) throws FitsException {
        ...
        for (int i = 0; i < nFields; i += 1) {
            ...
            String s = hdr.getTrimmedStringValue("TFORM" + (i + 1));
...
//      s = s.trim();
            ...
            // LAURENT : use getTrimmedStringValue instead of getStringValue:
            nulls[i] = hdr.getTrimmedStringValue("TNULL" + (i + 1));
//      if (nulls[i] != null) {
...
//      }
     ...
     * @throws FitsException if any IO exception is found or
     ...
     * @param newData   The new data.  Each element of this array is typically
     *                  a primitive[1] array.
     * @throws FitsException
     */
    ...
    /** Add a column to the table. Users should be cautious
     ...
     * @param newCol  the new column information.  This is typically a
     ...
     *  data.
     * @param hdr The header to be updated with information appropriate
     * to the current table data.
     */
    ...
    public void deleteColumns(int start, int len) throws FitsException {
        ...
        for (int i = start, size = start + len; i < size; i += 1) {
            rowLen -= (lengths[i] + 1);
        }
     ...
     * @param hdr       The associated header.
     * @throws FitsException
     */
